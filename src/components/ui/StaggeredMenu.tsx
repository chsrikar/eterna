...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  // Handle internal navigation (React Router) vs external links
  const handleItemClick = useCallback(
    (e: React.MouseEvent<HTMLAnchorElement>, link: string) => {
      // If it's an internal link (starts with /), use React Router
      if (link.startsWith('/')) {
        e.preventDefault();
        closeMenu();
        navigate(link);
      }
      // External links will follow normal <a> behavior
    },
    [navigate, closeMenu]
  );
          ...
          {logoUrl ? (
            ...
            <span
              style={{
                ...
                background: 'linear-gradient(135deg, #e8b4b8, #b5a3d4)',
